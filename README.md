...
VELA computes equilibrium stress autocorrelation functions in the time domain and converts them into modulus--frequency relationships from MD stress time series.
...
The default executable name is `VELA`.
...
```bash
./VELA INPUT_FILE OUTPUT_FILE_Gt OUTPUT_FILE_Gw DT MODE [GTCUT] [TAIL_PARAM] [--muA VALUE] [--muA-series FILE] [--muA-lammps-input FILE] [--muA-lammps-lib PATH] [--muA-lammps-numdiff DELTA] [--muA-lammps-channel CHANNEL] [--transform METHOD]
```
...
This backend is the current minimal autonomous `muA` route and the intended production path for absolute-modulus calculations.
...
The current VELA direct transform on the multi-tau grid. It preserves the established code path while reinterpreting the old k-sigma cutoff as a trusted-window detector.
...
- The autonomous `muA` route is **by design** the `liblammps` route: the user provides a **complete LAMMPS setup script** that already knows how to read the target `data` / `restart` / `dump` configuration and define the full force field. VELA then appends the Born/virial commands and executes `run 0`.
