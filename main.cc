...
void print_usage() {
    std::cout << "Usage:" << std::endl
              << "Program INPUT_FILE OUTPUT_FILE_G(T) OUTPUT_FILE_G(W) DT MODE [G(T)CUT] [TAIL_PARAM] [--muA VALUE] [--muA-series FILE] [--muA-lammps-input FILE] [--muA-lammps-lib PATH] [--muA-lammps-numdiff DELTA] [--muA-lammps-channel CHANNEL] [--transform METHOD]" << std::endl
              << "Note: VELA outputs raw reduced/absolute moduli. For LJ/reduced units use V/kBT; for LAMMPS pressure-unit input use V/(kB*T*nktv2p)." << std::endl
              << "      MODE==0: input columns are xy,xz,yz" << std::endl
              << "      MODE==1: input columns are xx,yy,zz,xy,xz,yz; mode-1 internally uses normal-stress differences and shear stresses" << std::endl
              << "      G(T)CUT sets a maximum time index for tail handling or transform range; defaults to the full time maximum" << std::endl
              << "      TAIL_PARAM is the k-sigma threshold for the statistical confidence cutoff / trusted-window detector" << std::endl
              << "      --muA VALUE switches to the absolute branch using the supplied raw affine modulus" << std::endl
              << "      --muA-series FILE switches to the absolute branch using the mean of a raw muA time/block series" << std::endl
